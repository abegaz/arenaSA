...
import arenaModels.ManageUsersController;
import arenaModels.TableViewController;
...
// Role IDs: #  ROLE
    ...
    private void signIn(ActionEvent event) throws SQLException, IOException {
    	...
    	// Role ID # = 1 (League Owner). Lands on "leagueOwnerLanding.fxml"
 ...
 // Spectator confirmation YES button Functionality
  ...
  // Then Delete that selection from pending Table. 
    ...
    private void approveNewUser(ActionEvent event) throws SQLException {
     ...
     String sqlUpdate ="UPDATE arenadatabase.users SET userRoleID = "+selectedDesiredRole+" WHERE users.userID = "+selectedUserID+" ";
...
// Operator Manage Users TableView. Populate Table from users Table when user presses Refresh Button. 
        ...
        private void ManageUsersToTableView(ActionEvent event) throws SQLException{
        	...
        	try {
        		ResultSet rs3 = myConnection.createStatement().executeQuery("SELECT userID,userName,userRoleID FROM arenadatabase.users");
        		while(rs3.next()) {
...
// Operator Manager Users TableView. Remove Button Functionality. Removes Selected user from Users Table. 
        ...
        @FXML
        private void GoToNewLeaguePage(ActionEvent event) throws Exception {
        	NewLeagueButton.getScene().getWindow().hide();
